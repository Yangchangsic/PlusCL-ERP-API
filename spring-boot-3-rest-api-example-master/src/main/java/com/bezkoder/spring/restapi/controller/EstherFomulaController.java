...
@RestController
@RequestMapping("/api")
public class EstherFomulaController {

    private static final Logger logger = LoggerFactory.getLogger(EstherFomulaController.class);

    @Autowired
    ApiService apiService;

    @GetMapping("/esther-formula")
    @ResponseBody
    public String getDataForB2C(String begin_date) {
        String category1 = "에스더포뮬러";
    ...
    @GetMapping("/esther-formula-excel")
    public ResponseEntity<byte[]> downloadExcel(String category1, String begin_date) throws IOException {
        List<Map<String, Object>> excelDataList = apiService.getDataForB2C(begin_date, category1);
